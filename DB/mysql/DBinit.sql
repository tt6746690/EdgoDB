...
CREATE TABLE IF NOT EXISTS `EdgoDB`.`Gene` (
  ...
  `UNIPROT_PROTEIN_LENGTH` INT NULL,
  `UNIPROT_PROTEIN_LOCALIZATION` VARCHAR(1000) NULL,
  `PROTEIN_ATLAS_LOCALIZATION` VARCHAR(1000) NULL,
  ...
  UNIQUE INDEX `BIOGRID_ID_UNIQUE` (`BIOGRID_ID` ASC))
ENGINE = InnoDB;
...
-- -----------------------------------------------------
-- Table `EdgoDB`.`ProteinDataBank`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `EdgoDB`.`ProteinDataBank` ;

CREATE TABLE IF NOT EXISTS `EdgoDB`.`ProteinDataBank` (
  `PROTEIN_DATA_BANK_ID` INT NOT NULL AUTO_INCREMENT,
  `PDB_ID` VARCHAR(45) NULL,
  `PDB_POSITION` VARCHAR(45) NULL,
  `UNIPROT_SWISSPROT_ID` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`PROTEIN_DATA_BANK_ID`),
  INDEX `fk_ProteinDataBank_Gene1_idx` (`UNIPROT_SWISSPROT_ID` ASC),
  CONSTRAINT `fk_ProteinDataBank_Gene1`
    FOREIGN KEY (`UNIPROT_SWISSPROT_ID`)
    REFERENCES `EdgoDB`.`Gene` (`UNIPROT_SWISSPROT_ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
