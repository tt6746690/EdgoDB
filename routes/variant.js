...
router.get('/', function(req, res, next){
  pool.getConnection(function(err, connection) {
    sqlstr = 'SELECT ??, ??, ??, ??, ?? FROM Gene JOIN Variant USING (ENTREZ_GENE_ID)'
    projectionList = ['ENTREZ_GENE_ID', 'HUGO_GENE_SYMBOL', 'MUT_HGVS_NT_ID', 'MUT_HGVS_AA_ID', 'CHR_COORDINATE_HG18']
...
router.get('/:variantid', function(req, res, next){
  pool.getConnection(function(err, connection) {
    sqlstr = "SELECT * \
              FROM Gene JOIN Variant USING (ENTREZ_GENE_ID) \
